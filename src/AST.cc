...
void AST::toLlvmIr() {
  ...
  for (StructExpr *stru_expr : _structs) {
    ...
    std::vector<DeclareVariableExpr *> attrs = stru_expr->attrs();
    int len = attrs.size();

    std::vector<llvm::Type *> *attr_types = new std::vector<llvm::Type *>();
    for (DeclareVariableExpr *stru_attr : attrs) {
...
void AST::add_concrete_struct(std::string &name,
  ...
  int id;
  std::unordered_map<std::string, StructExpr *>::iterator generic_struct_itr =
      _generic_structs.find(name);
  StructExpr *generic_struct = generic_struct_itr->second;
  ...
  if (itr._M_cur != nullptr) {
    ...
    if (id_itr._M_cur == nullptr) {
      ...
      StructExpr *concrete = generic_struct->generate_concrete(types, id);
...
void AST::add_concrete_struct_if_not_exist(StructExpr *expr) {
  std::vector<TypeExpr *> types = expr->all_concrete_struct_attr_type();

  for (TypeExpr *type : types) {
